...
import { Sankey, Node, ShadowNode, Flow, ShadowFlow } from './sankey.js';
import { LINE_COMMENT_REGEX, LINE_SETTINGS_REGEX, LINE_MOVE_REGEX, LINE_FLOW_REGEX, LINE_TSV_FLOW_REGEX, LINE_NODE_SETTINGS_REGEX, COLOR_GRAY_60, FONT_METRICS, HIGHLIGHT_STYLES } from './constants.js';
import { formatHTMLAsQuotedBold, el, elV } from './ux/dom.js'
import { togglePanel, updateLabelPositionBreakpoint, getLabelPositionBreakpointMax, setValueOnPage, getHumanValueFromPage, nudgeColorTheme, closeDialog, openGetLinkDialog, copyGeneratedLink, updateGeneratedLink, USER_INPUTS_FIELD } from './ux/controls.js'
import { parseValue, enoughPrecision as ep, contrastingGrayColor, escapeHTML, fileTimestamp, shiftArray } from './utils.js'
...
import { LEGACY_SETTINGS_POLYFILL, SANKEY_SETTINGS_SCHEMA, approvedColorTheme, settingIsValid, currConfig, settingHtoC, generateLink, getDiagramDefinition, extractFromUrl, REGEX_REMOVE_AUTO_LINES } from './sankeyConfig.js'
import { calculateMissingFlowValues, canonicalName, formatValue, FLOW_VALUE_CALCULATED} from './components/sankeyUtils.js'
...
function setUpNewInputs(newData, dataSource) {
  ...
  parseUserInputs(LEGACY_SETTINGS_POLYFILL + newData);
  // Reset breakpoint values to allow a high one in any imported diagram:
  updateLabelPositionBreakpoint();
  ...
  if (dataSource) {
    msg.addToQueue(`Imported diagram from ${dataSource}`);
  }
  el(USER_INPUTS_FIELD).dispatchEvent(new Event('change'));
}
...
function flatFlowPathMaker(flow) {
  const sx = flow.source.x + flow.source.dx; // source's trailing edge
  const tx = flow.target.x;                   // target's leading edge
  const syTop = flow.source.y + flow.sy;         // source flow top
  const tyBot = flow.target.y + flow.ty + flow.dy;  // target flow bottom

  flow.renderAs = 'flat'; // Render this path as a filled parallelogram
  ...
  // then [z] close.
  return `M${ep(sx)} ${ep(syTop)}v${ep(flow.dy)}L${ep(tx)} ${ep(tyBot)}v${ep(-flow.dy)}z`;
}
...
function curvedFlowPathFunction(curvature) {
  return flow => {
    const syC = flow.source.y + flow.sy + flow.dy / 2; // source flow's y center
    const tyC = flow.target.y + flow.ty + flow.dy / 2; // target flow's y center
    const sEnd = flow.source.x + flow.source.dx;    // source's trailing edge
    const tStart = flow.target.x;                // target's leading edge
    ...
    if (Math.abs(syC - tyC) < 2 || Math.abs(tStart - sEnd) < 12) {
      return flatFlowPathMaker(flow);
    }

    flow.renderAs = 'curved'; // Render this path as a curved stroke
    ...
    return `M${ep(sEnd)} ${ep(syC)}C${ep(xcp1)} ${ep(syC)} ${ep(xcp2)} ${ep(tyC)} ${ep(tStart)} ${ep(tyC)}`;
  };
}
...
globalThis.replaceGraph = async graphName => {
  ...
  if (!savedRecipe) {
    ...
    msg.add( `Requested sample diagram ${formatHTMLAsQuotedBold(graphName)} not found.`, 'issue');
    return null;
  }
...
function render_sankey(allNodes, allFlows, cfg) {
  const filteredNodes = cfg.internal_revealshadows ? allNodes : allNodes.filter(n => !(n instanceof ShadowNode))
  // Set up functions and measurements we will need:

  // withUnits: Format a value with the current style.
  function withUnits(n) {
    return formatValue(n, cfg);
  }
  ...
  function addTSpans(d3selection, textObjs, origSize, origX) {
    ...
    for (const tspan of textObjs) {
      ...
      if (tspan.newLine) {
        ...
        const lineSpacing = (0.95 + cfg.labels_linespacing) * ((prevLineMaxSize + tspan.size * 3) / 4);
  ...
  function measureSVGText(txtList, id) {
    if (!txtList?.length) return { w: 0, h: 0, line1h: 0 };

    const firstEl = txtList[0];
    const laterSpans = txtList.slice(1);
    const firstNewLineIndex = laterSpans.findIndex(tspan => tspan.newLine);
    const line1Weight = firstEl.weight ?? cfg.labelname_weight;
    ...
    let line1Suffixes = [];
    let laterLines = [];
    let line1Size = firstEl.size ?? cfg.labelname_size;
  ...
  function setUpTextDimensions() {
    ...
    const boundingBoxH = emSize.h; // (same for all characters)
    const emW = emSize.w;
    // The WIDTH of an 'x' is a crude estimate of the x-HEIGHT, but it's what we have for now:
    const exH = measureSVGText([{ txt: 'x' }], 'ex').w;
    // Firefox has unique SVG measurements in 2022, so we look for it:
    const browserKey = isFirefox() ? 'firefox' : '*';
    const metrics = FONT_METRICS[browserKey][cfg.labels_fontface] || FONT_METRICS[browserKey]['*'];
    const m = {
    ...
    m.lblMarginBefore = (cfg.node_border / 2) + (metrics.marginRight + metrics.marginAdjLeft) * m.inner;
    return m;
  }
  ...
  // etc.
  const sankeyObj = new Sankey(allNodes, allFlows, cfg);
  ...
  const oldMax = getLabelPositionBreakpointMax();
  ...
  function shadowFilter(i) {
    return cfg.internal_revealshadows || !(i instanceof ShadowFlow);
  }

  if (cfg.internal_revealshadows) {
    // Add a usable tipname since they'll be used (i.e. avoid 'undefined'):
    for (const n of allNodes.filter(n => n instanceof ShadowNode)) {
      n.tipname = '(shadow)';
    }
  }
  ...
  function getLabelPieces(n, magnification) {
    if (!n.rawName) return [];
    if (/^-.*-$/.test(n.rawName)) return [];

    const overallSize = cfg.labelname_size * magnification;
    // The relative-size values 50 to 150 become -.5 to .5:
    const relativeSizeAdjustment = (cfg.labels_relativesize - 100) / 100;
    const nameSize = overallSize * (1 - relativeSizeAdjustment);
    const valueSize = overallSize * (1 + relativeSizeAdjustment);
    const total = cfg.labelpercentage_total === 'parent'
      ? n.incomingFlows.reduce((prev, cur) => {
        return prev + cur.source.value;
      }, 0)
      : allNodes.reduce((prev, cur) => {
        return prev + (cur.incomingFlows.length == 0 ? cur.value : 0);
      }, 0);
    const percentage = cfg.labelpercentage_appears && total != 0
      ? String(Number.parseFloat((n.value / total * 100).toFixed(cfg.labelpercentage_precision))) + '%'
      : '';
    const nameParts = String(n.rawName).split('\\n'); // Use \n for multiline labels
    const nameObjs = nameParts.map((part, i) => ({
      txt: part,
      weight: cfg.labelname_weight,
      size: nameSize,
      newLine: i > 0 || ((cfg.labelvalue_appears || cfg.labelpercentage_appears) && cfg.labelvalue_position === 'above'),
    }));
    const valObj = {
      txt: (cfg.labelvalue_appears ? withUnits(n.value) : '') +
          (cfg.labelpercentage_appears && percentage != '' ? (cfg.labelvalue_appears ? ` (${percentage})` : percentage) : ''),
  ...
  function labelAnchor(n) {
    ...
    if (!n.incomingFlows.length) {
      return 'end';
    }
    if (!n.outgoingFlows.length) {
      return 'start';
    }
  ...
  const nodeScaleFn = v => (minVal === maxVal ? 1 : (v - minVal) / (maxVal - minVal)); // returns a Number from 0 to 1:
  ...
  if (cfg.labelname_appears || cfg.labelvalue_appears || cfg.labelpercentage_appears) {
    for (const n of filteredNodes) {
      ...
      const nFactor = nodeScaleFn(n.value);
      const nAbsolutePos = cfg.labels_magnify >= 100 ? nFactor : 1 - nFactor;
      // Locate this value in the overall range of sizes, then
      // scoot that range to be centered on 0:
      const nodePositionInRange = nAbsolutePos * totalRange - totalRange / 2;
      const magnifyLabel = cfg.labels_magnify === 100 ? 1 : 1 + nodePositionInRange;
      const id = `label_${n.id}`; // label0, label1..
  ...
  function maxLabelWidth(stageArr, labelsBefore) {
    let maxWidth = 0;
    for (const node of stageArr.filter(node => node.labelList?.length)) {
      const labelTotalW = node.label.bb.w + (labelsBefore ? pad.lblMarginBefore : pad.lblMarginAfter) + pad.outer;
  ...
  function setUpDiagramSize() {
    ...
    const lastStage = stagesArr.length - 1;
    const labelsBeforeFirst = stagesArr[0].filter(n => n.label?.anchor === 'end');
    const labelsAfterLast = stagesArr[lastStage].filter(n => n.label?.anchor === 'start');
    // If any labels are BEFORE stage 0, get its maxLabelWidth:
    const leadingW = labelsBeforeFirst.length > 0 ? maxLabelWidth(stagesArr[0], true) : cfg.node_border / 2;
    // If any labels are AFTER the last stage, get its maxLabelWidth:
    const trailingW = labelsAfterLast.length > 0 ? maxLabelWidth(stagesArr[lastStage], false) : cfg.node_border / 2;
    // Compute the ideal width to fit everything successfully:
    const idealW = graphW - leadingW - trailingW;
    // Find the smallest width we will allow -- all the Node widths
    // plus (5px + node_border) for every Flow region:
    const minimumW = (stagesArr.length * cfg.node_w) + (lastStage * (cfg.node_border + 5));
    // Pick which width we will actually use:
    const w = Math.max(idealW, minimumW);

    const h = cfg.size_h - cfg.margin_t - cfg.margin_b;
    ...
    const leadingShareOfError = leadingW + trailingW > 0 ? (leadingW / (leadingW + trailingW)) : 0.5;
    // The actual amount of error (if any) for the leading side:
    const leadingCutOffAdjustment = idealW < minimumW ? (idealW - minimumW) * leadingShareOfError : 0;
    const leftMargin = cfg.margin_l + leadingW + leadingCutOffAdjustment;
    return { w, h, leftMargin };
  }
  ...
  sankeyObj.layout(); // Note: The 'layout()' step must be LAST
  ...
  const colorset = approvedColorTheme(cfg.node_theme).colorset;
  const themeOffset = cfg[`themeoffset_${cfg.node_theme}`];
  const userColorArray = cfg.node_theme === 'none' ? [cfg.node_color] : shiftArray(colorset, themeOffset);
  const colorScaleFn = d3.scaleOrdinal(userColorArray);
  ...
  const flowsAreFlat = (cfg.flow_curvature <= 0.1);
  // flowPathFn is a function producing an SVG path; the same function is
  // used for all Flows. (Flat flows use a simpler function.)
  const flowPathFn = flowsAreFlat ? flatFlowPathMaker : curvedFlowPathFunction(cfg.flow_curvature);
  // Is the diagram background dark or light?
  const darkBg = (cfg.bg_color.toUpperCase() < '#888');
  // Is the label color more like black or like white?
  const darkLabel = (cfg.labels_color.toUpperCase() < '#AAA');
  // Set up label highlight values:
  const hlStyle = HIGHLIGHT_STYLES[darkLabel ? 'dark' : 'light'];
  ...
  for (const n of filteredNodes) {
    ...
    // dragged:
    n.css_class = `for_${n.id}`; // for_r0, for_r1...
    n.tooltip = `${n.name}:\n${withUnits(n.value)}`;
    n.opacity ??= cfg.node_opacity;
    ...
    if (!n.color) {
      ...
      const colorKeyString = n.name.split(/\s/, 1)[0] || 'name-is-blank';

        // Don't use up colors on shadow nodes:
      n.color = n instanceof ShadowNode ? COLOR_GRAY_60 : colorScaleFn(colorKeyString);
    }
    // Now that we're guaranteed a color, we can calculate a border shade:
    n.border_color = darkBg ? d3.rgb(n.color).brighter(2) : d3.rgb(n.color).darker(2);

    // Set up label presentation values:
    if (n.labelList?.length) {
      ...
      if (hlStyle.orig.fill_opacity > 0) {
        n.label.bg = {
          id: `${n.label.id}_bg`, // label0_bg, label1_bg..
          offset: {
  ...
  for (const f of allFlows.filter(shadowFilter)) {
    f.tooltip = `${f.source.name} → ${f.target.name}: ${withUnits(f.value)}`;
    // Fill in any missing opacity values and the 'hover' counterparts:
    f.opacity ??= cfg.flow_opacity;
    ...
    if (!f.color) {
      ...
      if (f instanceof ShadowFlow) {
        f.color = COLOR_GRAY_60;
      } else if (f.source.paintOutgoing) {
        f.color = f.source.color;
      } else if (f.target.paintIncoming) {
  ...
  const diagMain = diagramRoot.append('g')
    .attr('transform', `translate(${ep(graph.leftMargin)},${ep(cfg.margin_t)})`);
  ...
  function applyFlowEffects(f, o, s) {
    ...
    d3.select(`#${f.id}`).attr('opacity', o);
    [f.source, f.target].filter(n => n.label?.bg)
      .forEach(n => {
        d3.select(`#${n.label.bg.id}`)
          .attr('fill', s.fill)
  ...
  const diagFlows = diagMain.append('g')
    ...
    .sort((a, b) => ((b instanceof ShadowFlow) - (a instanceof ShadowFlow)) || (b.dy - a.dy));
  ...
  function applyNodeMove(n) {
    // In the case of a reversed graph, we negate the x-move:
    const myXMove = n.move[0] * (cfg.layout_reversegraph ? -1 : 1);
    const availableW = graph.w - n.dx;
    const availableH = graph.h - n.dy;
  ...
  function dragNodeStarted(event, n) {
    ...
    if (!(event.sourceEvent && event.sourceEvent.shiftKey)) {
      ...
      const hintHeights = graph.h > 350 ? [0.05, 0.95] : [0.4];
      // Show the text so it's visible but not overwhelming:
      for (const h of hintHeights) {
  ...
  function dragNodeEnded(event, n) {
    ...
    for (const f of allFlows.filter(f => f.hovering)) {
  ...
  if (cfg.meta_mentionsankeymatic) {
    ...
    const mMargin = Math.round(mSize / 2) - 1;
    const mColor = cfg.bg_color === '#ffffff' ? '#336781' : contrastingGrayColor(cfg.bg_color);
    ...
    // have to use the whole width & adjust for the graph's transform:
      .attr('x', ep(cfg.size_w / 2 - graph.leftMargin))
  ...
  if (!cfg.labels_hide && (cfg.labelname_appears || cfg.labelvalue_appears || cfg.labelpercentage_appears)) {
    ...
    diagLabels.selectAll()
      .data(filteredNodes)
      .enter()
      .filter(n => n.labelList?.length > 0)
      .append('text')
      .attr('id', n => n.label.id)
    ...
    for (const n of filteredNodes.filter(n => n.label?.bg)) {
      // Use each label's size to make custom round-rects underneath:
      const labelTextSelector = `#${n.label.id}`;
      const labelBB = diagLabels.select(labelTextSelector).node().getBBox();
      const bg = n.label.bg;
  ...
  if (currConfig.rememberedMoves.size) {
    const allNodesMap = new Map(allNodes.map(n => [n.name, n]));

    // Make a copy of the list of moved-Node names (so we can destroy it):
    // Look for all node objects matching a name in the list:
    for (const [nodeName, move] of currConfig.rememberedMoves.entries()) {
      const n = allNodesMap.get(nodeName);
      if (n) {
        n.move = move;
...
globalThis.openGetLinkDialog = () => {
  openGetLinkDialog();
  updateGeneratedLink(generateLink());
};
...
function parseUserInputs(updateInputValue) {
  function warnAbout(line, warnMsg) {
    msg.addToQueue(`${warnMsg}: ${formatHTMLAsQuotedBold(line)}`, 'issue');
  }
  ...
  const existingInput = elV(USER_INPUTS_FIELD)
  const sourceLines = (updateInputValue || existingInput).split('\n');

  const linesToKeep = [];
  const nodeLines = [];
  const nodeSettingLines = [];
  const moveLines = [];
  const settingsLines = [];
  for (let lineIn of sourceLines) {
    lineIn = lineIn.replace(/[\u200B-\u200D\uFEFF]/g, '');
    let match;
    // if is a comment (keep)
    // test if it is a node/data row (keep)
    // test if it is a node setting (keep)
    // test if it is a move row (move)
    // test if it is a settings row (move)
    // if it is none of the above, it is invalid (keep)
    if (REGEX_REMOVE_AUTO_LINES.test(lineIn)) {
      // noop;
    } else if (lineIn.match(LINE_COMMENT_REGEX)) {
      linesToKeep.push(lineIn.trim());
    } else if (lineIn.trim() === '') {
      if (linesToKeep.length > 0) linesToKeep.push(lineIn.trim());
    } else if (match = lineIn.match(LINE_FLOW_REGEX)) {
      linesToKeep.push(lineIn);
      nodeLines.push([linesToKeep.length - 1, match]);
    } else if (match = lineIn.match(LINE_TSV_FLOW_REGEX)) {
      linesToKeep.push(lineIn);
      nodeLines.push([linesToKeep.length - 1, match]);
    } else if (match = lineIn.match(LINE_NODE_SETTINGS_REGEX)) {
      linesToKeep.push(lineIn);
      nodeSettingLines.push([linesToKeep.length - 1, match]);
    } else if (match = lineIn.match(LINE_MOVE_REGEX)) {
      moveLines.push(match);
    } else if (match = lineIn.match(LINE_SETTINGS_REGEX)) {
      settingsLines.push(match);
    } else {
      linesToKeep.push(lineIn.trim());
      warnAbout(lineIn, 'Invalid line');
    }
  }
  // trim blank lines at the start & end
  const newInput = linesToKeep.join('\n').replace(/^\n+/, '').replace(/\n+$/, '');
  if (newInput !== existingInput) el(USER_INPUTS_FIELD).value = newInput;

  for (const moveParts of moveLines) {
  ...
  const default_max_contained = { w: elV('size_w'), h: elV('size_h') };
  let currentSettingGroup = '';
  for (const settingParts of settingsLines) {
    let {group, name, value} = settingParts.groups;
    ...
    currentSettingGroup = group || currentSettingGroup;
    const settingName = `${currentSettingGroup}_${name}`;

    const settingData = SANKEY_SETTINGS_SCHEMA.get(settingName);
    if (!settingData) {
      warnAbout(`${group || ''} ${name}`, 'Not a valid setting name');
      continue;
    }
    // Update the group-prefix, whether or not the value validates
    // below. (Better to honor this prefix than to use one from
    // further up.):
    const dataType = settingData[0];
    const sizeObj = dataType === 'contained' ? default_max_contained : {};
    const finalValue = settingIsValid(settingData, value, sizeObj);
    if (typeof finalValue === 'undefined') {
      // The setting exists but the value wasn't right:
      warnAbout(value, `Invalid value for <strong>${settingName}<strong>`);
    }
    else {
  ...
  return {nodeLines, nodeSettingLines};
}
...
export function process_sankey () {
   ...
   * @param {string} rawName A raw node name from the input data
   * @param {number} sourceRow The number of the input row the node appeared on.
   *  (This can be a non-integer; Target node names have 0.5 added to their
   *  row number.)
   * @returns {object} The node's object (from uniqueNodes)
   */
  function setUpNode(rawName, sourceRow) {
    const currentNode = allNodes.get(canonicalName(rawName)); // Does this node exist?
    if (currentNode) {
      // If so, should the new row # replace the stored row #?:
      currentNode.sourceRow = Math.min(currentNode.sourceRow, sourceRow);
      // Use the more formatted name. Use length as a proxy for 'more formatted':
      if (rawName.length > currentNode.rawName.length) {
        currentNode.rawName = rawName;
      }
      return currentNode;
    }
    // This is a new Node. Set up its object, keyed to its trueName:
    const newNode = new Node(rawName, sourceRow);
    allNodes.set(newNode.name, newNode);
    return newNode;
  }
  ...
  function warnAbout(line, warnMsg) {
    msg.add(`${warnMsg}: ${formatHTMLAsQuotedBold(line)}`, 'issue');
  }

  // loop through the inputs and itentity rows we want to keep vs rows that should be moved to the UX settings
  const {nodeLines, nodeSettingLines} = parseUserInputs();
  msg.showQueued();

  //  Parse inputs into: approvedNodes, approvedFlows
  let maxDecimalPlaces = 0;
  for (const [sourceRow, match] of nodeLines) {
    const lineIn = match[0];
    let {source, value, target, color, opacity} = match.groups;
    const trialTarget = parseValue(target, FLOW_VALUE_CALCULATED);
    const trialValue = parseValue(value, FLOW_VALUE_CALCULATED);

    // are the target and value fields reversed?
    // TODO: check this once and remember for consistency?
    if (trialValue === null && trialTarget !== null) {
      target = value;
      value = trialTarget;
    } else {
      value = trialValue;
    ...
    if (value === null) {
      warnAbout(lineIn, `The [Amount] must be a number in the form #.# or a wildcard ("*" or "?").`);
      continue;
    }
    ...
    const sourceNode = setUpNode(source, sourceRow);
    const targetNode = setUpNode(target, sourceRow + 0.5);
    const thisFlow = new Flow(sourceNode, targetNode, value, sourceRow, color, opacity);
    allFlows.push(thisFlow);
    ...
    maxDecimalPlaces = Math.max( maxDecimalPlaces, ((value + '').split('.')[1] || '').length );
  }

  for (const [sourceRow, match] of nodeSettingLines) {
    const {nodeName, color, opacity, paintIncoming, paintOutgoing} = match.groups;
    const name = canonicalName(nodeName);
    ...
    const thisNode = allNodes.get(name);
    if (!thisNode) {
      warnAbout(nodeName, 'Node not found');
      continue;
    }

    if (color) thisNode.color = color[0] !== '#' ? `#${color}` : color;
    if (opacity) thisNode.opacity = opacity;
    thisNode.paintIncoming = Boolean(paintIncoming);
    thisNode.paintOutgoing = Boolean(paintOutgoing);
    thisNode.sourceRow = Math.min(thisNode.sourceRow, sourceRow);

    // If there's a color and it's a color CODE, put back the #:
    // TODO: honor or translate color names?
  }

  calculateMissingFlowValues(allNodes.values(), allFlows);
  ...
  const graphIsReversed = el('layout_reversegraph').checked;
  if (graphIsReversed) {
    for (const thisFlow of allFlows) {
      [thisFlow.source, thisFlow.target] = [thisFlow.target, thisFlow.source];
    }
    for (const thisNode of allNodes.values()) {
      [thisNode.incomingFlows, thisNode.outgoingFlows] = [thisNode.outgoingFlows, thisNode.incomingFlows];
      // Set up color inheritance signals from '<<' and '>>' indicators:
      [thisNode.paintIncoming, thisNode.paintOutgoing] = [thisNode.paintOutgoing, thisNode.paintIncoming];
    }
  }
  ...
  for (const [fldName, fldData] of SANKEY_SETTINGS_SCHEMA) {
    const [dataType, defaultVal] = fldData;
    const fldVal = getHumanValueFromPage(fldName, dataType);
    const sizeObj = dataType === 'contained' ? { w: approvedCfg.size_w, h: approvedCfg.size_h } : {};
    // Consult the oracle to know if it's a good value:
    const finalValue = settingIsValid(fldData, fldVal, sizeObj);
    if (typeof finalValue === 'undefined') {
  ...
  approvedCfg.maxDecimalPlaces = maxDecimalPlaces;
  ...
  for (const s of [1, 2, 4, 6]) {
    el(`save_as_png_${s}x`).title = `PNG image file: ${approvedCfg.size_w * s} x ${approvedCfg.size_h * s}`;
  }

  // Were there any good flows at all? If not, offer a little help and then EXIT EARLY:
  if (!allFlows.length) {
    msg.add('Enter a list of Flows &mdash; one per line. See the <a href="/manual/" target="_blank">Manual</a> for more help.');
  ...
  render_sankey(Array.from(allNodes.values()), allFlows, approvedCfg);

  // MARK Post-Render Activity - various stats & message updates.
  updateStatsOutput(allNodes, allFlows, approvedCfg);
...
function updateStatsOutput(uniqueNodes, approvedFlows, approvedCfg) {
  function withUnits(n) {
    return formatValue(n, approvedCfg);
  }

  // explainSum: Returns an html string showing the flow amounts which
  // add up to a node's total value in or out.
  function explainSum(n, flows, flowsTotal) {
    const formattedSum = withUnits(flowsTotal);
    const flowGroup = flows.filter(f => !(f instanceof ShadowFlow));
    const flowCt = flowGroup.length;
    if (flowCt === 1) {
      return formattedSum;
    }
    ...
    const breakdown = flowGroup.map(f => withUnits(f.value)).join(' + ');
    return `<dfn title="${formattedSum} from ${flowCt} Flows: ${breakdown}">${formattedSum}</dfn>`;
  }
  ...
  const epsilonDifference = 10 ** (-approvedCfg.maxDecimalPlaces - 1);
  const differences = [];
  const grandTotal = { incomingTotal: 0, outgoingTotal: 0 };
  ...
  for (const n of uniqueNodes.values()) {
    ...
    if (n.incomingFlows.length && n.outgoingFlows.length) {
      const difference = n.incomingTotal - n.outgoingTotal;
      ...
      if (Math.abs(difference) > epsilonDifference) {
        differences.push({
          name: n.name,
          totalIncoming: explainSum(n, n.incomingFlows, n.incomingTotal),
          totalOutgoing: explainSum(n, n.outgoingFlows, n.outgoingTotal),
      ...
      grandTotal.outgoingTotal += n.incomingTotal;
      grandTotal.incomingTotal += n.outgoingTotal;
    }
  }
  const maxNode = uniqueNodes.values().reduce((max, node) => node?.value > max?.value ? node : max);
  ...
  el('meta_listimbalances').disabled = disableDifferenceControls;
  el('layout_attachto_leading').disabled = disableDifferenceControls;
  el('layout_attachto_trailing').disabled = disableDifferenceControls;
  el('layout_attachto_nearest').disabled = disableDifferenceControls;

  el('imbalances_area').ariaDisabled = disableDifferenceControls;
  ...
  if (!disableDifferenceControls && approvedCfg.meta_listimbalances) {
    ...
    for (const diffRec of differences) {
      differenceRows.push(
        `<tr><td class="nodename">${escapeHTML(diffRec.name)}</td>`
        + `<td>${diffRec.totalIncoming}</td>`
        + `<td>${diffRec.totalOutgoing}</td>`
        + `<td>${diffRec.difference}</td></tr>`
      );
    }
    msg.add(`<table class="center_basic">${differenceRows.join('\n')}</table>`, 'difference');
  }

  // Reflect summary stats to the user:
  let totalsMsg = `<strong>${approvedFlows.length} Flows</strong> between <strong>${uniqueNodes.size} Nodes</strong>. `;

  // Do the totals match? If not, mention the different totals:
  if (Math.abs(grandTotal.incomingTotal - grandTotal.outgoingTotal) > epsilonDifference) {
    const gtLt = grandTotal.incomingTotal > grandTotal.outgoingTotal ? '&gt;' : '&lt;';
    totalsMsg += `Total Inputs: <strong>${withUnits(grandTotal.incomingTotal)}</strong> ${gtLt} Total Outputs: <strong>${withUnits(grandTotal.outgoingTotal)}</strong>`;
  } else {
    totalsMsg += `Total Inputs = Total Outputs = <strong>${withUnits(grandTotal.incomingTotal)}</strong> &#9989;`;
  }
  ...
  const tallestNodeHeight = parseFloat(el(maxNode.id).getAttributeNS(null, 'height'));
  ...
  const unitsPerPixel = maxNode.value / (tallestNodeHeight || Number.MIN_VALUE);
  const unitsPerPixelFormatted = formatValue(unitsPerPixel, { ...approvedCfg, maxDecimalPlaces: 4 });
  el('scale_figures').innerHTML = `<strong>${unitsPerPixelFormatted}</strong> per pixel (${withUnits(maxNode.value)}/${formattedPixelCount}px)`;
}
